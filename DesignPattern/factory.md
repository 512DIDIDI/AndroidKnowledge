...
* **为什么要用工厂模式(何时使用，解决了什么问题)**
  ...
  当出现了大量的`if-else(when)`**创建不同但是相关类型的对象(继承同一父类或者接口的一组子类)**时，可以考虑使用工厂模式来优化代码。
...
* **怎么实现简单工厂和工厂模式(工厂模式是如何演变过来的)**
  ...
  /**
   * step1：一般写法，根据不同的配置信息，使用if-else或者when来动态创建对象
   * 缺点：代码臃肿，不同业务功能代码耦合。
   */
  class ParserLoader1 {
...
* **工厂模式  策略者模式   建造者模式的区别**
  ...
  工厂模式是**不同对象**创建 / 策略者模式是**不同行为**构建 / 建造者模式是**一个对象**的复杂创建过程
